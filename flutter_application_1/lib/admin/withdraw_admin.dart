...
class WithdrawAdmin extends StatefulWidget {
  const WithdrawAdmin({super.key});

  @override
  State<WithdrawAdmin> createState() => _WithdrawAdminState();
}

class _WithdrawAdminState extends State<WithdrawAdmin> {
  @override
  void initState() {
    super.initState();
      ...
      for (var doc in querySnapshot.docs) {
        ...
        if (data['id'] != 'admin@stockup.com') {
          ...
          var operation = firestore
              ...
              .get()
              .then((value) {
            double funds = value.data()!['current_request'] != null ? double.tryParse(value.data()!['current_request'])! : 0;
...
*/
  ...
  _withdrawAmount(String key, String amount) {
    try {
      firestore
          .collection('users')
          .doc(key)
      ...
      firestore
        ...
        if (documentSnapshot.exists) {
          ...
          double currentAmount =
              double.tryParse(data['available_fund'])! - double.tryParse(amount)!;
          await firestore
              .collection('users')
              .doc(key)
              .collection('details')
              .doc('wallet')
              .update({
            'available_fund': currentAmount.toString(),
